...
async def run_interaction(client: AsyncDLightClient, target_ip: str, device_id: str):
    ...
    try:
        ...
        log.info(
            f"  Device Info: Model={info.get('deviceModel')}, SW={info.get('swVersion')}, HW={info.get('hwVersion')}"
        )
...
async def main():
    ...
    parser.add_argument("--discover", action="store_true", help="Discover dLight devices on the network.")
    parser.add_argument(
        "--discover-duration", type=float, default=3.0, help="Duration (seconds) to listen for discovery responses."
    )
    parser.add_argument("--ip", type=str, default=None, help="IP address of the target dLight device for interaction.")
    parser.add_argument(
        "--id",
        type=str,
        default=None,
        dest="device_id",  # Use dest for clarity
        help="Device ID of the target dLight device for interaction.",
    )
    parser.add_argument("--timeout", type=float, default=5.0, help="Network timeout (seconds) for TCP commands.")
    parser.add_argument(
        "--connect-wifi",
        action="store_true",
        help="Attempt to send Wi-Fi credentials (requires --id, --ssid, --password).",
    )
    parser.add_argument("--ssid", type=str, default=None, help="Target Wi-Fi SSID for --connect-wifi.")
    parser.add_argument("--password", type=str, default=None, help="Target Wi-Fi password for --connect-wifi.")
    parser.add_argument(
        "--first",
        action="store_true",
    ...
    if args.discover:
        ...
        log.info(
            "No action specified (or discovery ran without --first). "
            "Use --discover, --connect-wifi, or provide --ip and --id."
        )
